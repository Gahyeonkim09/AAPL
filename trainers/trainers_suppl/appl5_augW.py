...
class DistortImage(nn.Module):
    ...
    def forward(self, image , augment_type_1, augment_type_2): 
        ...
        # image : [3, 224, 224]
        # if augment_type_1 == 'sobel_filter' or augment_type_2 == 'sobel_filter': pdb.set_trace()
...
class DiVE(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.aug_selected_count = {key: 0 for key in self.augment_types}
    ...
    def weighted_random_sample_no_repeats(self, items, weights, k):
        selected_items = set()
        while len(selected_items) < k:
            choice = random.choices(items, weights)[0]
            selected_items.add(choice)
        return list(selected_items)
    
    def forward(self, image, label=None, aug_weight=None):    
        ...
        if self.training:
            ...
            if sum(aug_weight)==13:    # 첫번째 epoch
                augment_type_1, augment_type_2 = random.sample(self.augment_types, 2)
            else:
                augment_type_1, augment_type_2 = self.weighted_random_sample_no_repeats(self.augment_types, aug_weight, 2)
                self.aug_selected_count[augment_type_1] += 1
                self.aug_selected_count[augment_type_2] += 1
            ...
            # total_loss = ad_tri_loss + ce_loss 
        ...
        if not self.cfg.TEST.NO_TEST:
            ...
            # pdb.set_trace() 
            return logits, tokens_list, self.aug_selected_count

        return logits

#  # Augmentation Type learner 
@TRAINER_REGISTRY.register()
class APPL5_augW(TrainerX):
    def __init__(self, cfg):
        super().__init__(cfg)
        self.augment_types = ['color_jitter_red', 'color_jitter_green', 'color_jitter_blue', 'grayscale', 'gaussian_noise', 'gaussian_blur','horizontal_flip', 
                              'vertical_flip', 'rotation_90', 'rotation_180', 'rotation_270', 'crop', 'cutout', 'sobel_filter']
        self.aug_weight = [1] * 13   # aug type별 sampling 가중치 
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            with autocast():
                total_loss, ce_loss, ad_tri_loss, ad_distances = model(image, label, self.aug_weight)
    ...
    def after_epoch(self):
        ...
        if do_test and self.cfg.TEST.FINAL_MODEL == "best_val":
            if last_epoch :  # 마지막 에포크에서만 test 
                ...
                # pdb.set_trace()
                self.aug_weight = self.AUG_SCORE()
                # self.AUG_SCORE()
                self.TSNE()
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            if batch_idx == 0:
                output, tokens_list, aug_selected_count = self.model_inference(input)
                self.tokens_list = tokens_list
                self.aug_selected_count = aug_selected_count
            else:
    ...
    def AUG_SCORE(self):
        ...
        softmax_scores = softmax(-np.array(cluster_silhouette_avg_list)/0.1)
        self.aug_weight = softmax_scores

        file_name = f'{self.output_dir}/aug_selected_count.json'
        try:
            with open(file_name, 'r', encoding='utf-8') as file:
                data = json.load(file)
        except FileNotFoundError:
            data = {}
        data[f'epoch_{self.epoch+1}'] = self.aug_selected_count
        data[f'epoch_{self.epoch+1}']['weight'] = self.aug_weight.tolist()
        data[f'epoch_{self.epoch+1}']['silhouette_avg'] = silhouette_avg_all
