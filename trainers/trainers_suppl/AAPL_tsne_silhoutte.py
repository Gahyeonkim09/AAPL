...
class DiVE(nn.Module):
    ...
    def forward(self, image, label=None):    
        ...
        if not self.cfg.TEST.NO_TEST:
            ...
            label_list = label.repeat(1, 15).flatten()
            return logits, tokens_list, label_list
        
        return logits

# Augmentation Type learner 
@TRAINER_REGISTRY.register()
class APPL_TSNE(TrainerX):
    def __init__(self, cfg):
        super().__init__(cfg)
        self.augment_types = ['color_jitter_red', 'color_jitter_green', 'color_jitter_blue', 'grayscale', 'gaussian_noise', 'gaussian_blur','horizontal_flip', 
                            'vertical_flip', 'rotation_90', 'rotation_180', 'rotation_270', 'crop', 'cutout', 'sobel_filter']
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output, tokens_list, label_list = self.model(input, label)
            # output, bias_list = self.model_inference(input, label)
            if batch_idx == 0:
                ...
                # pdb.set_trace()
                meta_tokens_list = torch.cat((self.tokens_list[0], tokens_list[0]), dim=1)      # [15, 100, 512] >> [15, 200, 512]
                delta_tokens_list = torch.cat((self.tokens_list[1], tokens_list[1]), dim=1)
                self.tokens_list = [meta_tokens_list, delta_tokens_list]
                # pdb.set_trace()
                self.label_list = torch.cat((self.label_list, label_list), dim=0)
    ...
    def TSNE(self):
        # augmentation 총 14가지 
        augment_types = ['color_jitter_red', 'color_jitter_green', 'color_jitter_blue', 'grayscale', 'gaussian_noise', 'gaussian_blur','horizontal_flip', 
                        'vertical_flip', 'rotation_90', 'rotation_180', 'rotation_270', 'crop', 'cutout', 'sobel_filter']
        # os.makedirs(f'{self.output_dir}/tsne_meta', exist_ok=True)
        os.makedirs(f'{self.output_dir}/tsne_delta_all', exist_ok=True)
        os.makedirs(f'{self.output_dir}/tsne_meta_class', exist_ok=True)
        meta_tokens_list, delta_meta_list = self.tokens_list                             # [15, 20, 512], [14, 20, 512]
        # label_list = self.label_list.cpu()
        val_N = meta_tokens_list.size(1)
        print('######## validation image 개수 :', val_N)
        # meta_tokens_list : original + aug 14개 -> 15개 각각 이미지 20개마다 적용.  
        # meta_token_data : [0]~[19]=한가지 aug, 이미지 20개씩 15개 순서. 
        meta_token_data = meta_tokens_list.cpu().detach().view(-1, 512).numpy()                 # (15,20,512)>>(300,512)
        delta_token_data = delta_meta_list.cpu().detach().view(-1, 512).numpy()                 # (14,20,512)>>(280,512)

        # pdb.set_trace()
        tsne_model = TSNE(n_components=2, perplexity=10, verbose=1, random_state=123)
        # tsne_results_1 = tsne_model.fit_transform(meta_token_data)                              # (300, 2) -> 각각 x, y 좌표 
        tsne_results_2 = tsne_model.fit_transform(delta_token_data)                             # (280, 2)
        # pdb.set_trace()

        colors = cm.rainbow(np.linspace(0, 1, len(augment_types) + 1))                          # (15, 4)
        # plt.figure(figsize=(12, 8))
        # for i, aug_type in enumerate(['original_image'] + augment_types):
        #     indices = (i*val_N, (i+1)*val_N)                                                    # (0, 20) (20, 40) ...
        ...
        plt.legend()
        plt.savefig(f'{self.output_dir}/tsne_delta_all/{self.epoch+1}_delta_aug.png')
        ...
        def generate_unique_colors(num_colors):
            return [mcolors.hsv_to_rgb((i / num_colors, 1, 1)) for i in range(num_colors)]
        
        # meta_tokens_list : original + aug 14개 -> 15개 각각 이미지 20개마다 적용.  
        original_meta_token = meta_tokens_list[0,:].cpu().detach().numpy()               
        # pdb.set_trace()
        tsne_model = TSNE(n_components=2, perplexity=10, verbose=1, random_state=123)
        tsne_results_1 = tsne_model.fit_transform(original_meta_token)            
        origin_label = self.label.cpu()

        unique_labels = np.unique(origin_label)       
        num_labels = len(unique_labels)
        unique_colors = generate_unique_colors(num_labels)
        # pdb.set_trace()

        plt.figure(figsize=(12, 8))
        for i, label in enumerate(unique_labels):
            indices = np.where(origin_label == label)[0]
            # pdb.set_trace()
            plt.scatter(tsne_results_1[indices, 0], tsne_results_1[indices, 1], color=unique_colors[i], label=f'Class {label}')
            plt.title(f'epoch_{self.epoch+1} Meta_tokens - class')
            # plt.legend(fontsize="xx-small")
            plt.savefig(f'{self.output_dir}/tsne_meta_class/{self.epoch+1}_meta_class.png')
        ...
        #     plt.scatter(tsne_results_2[indices, 0], tsne_results_2[indices, 1], color=cmap(label), label=f'Class {label}')
        #     plt.title(f'epoch_{self.epoch+1} Delta_meta_tokens - label')
        #     plt.legend()
        #     plt.savefig(f'{self.output_dir}/tsne_delta/{self.epoch+1}_delta_class.png')
    ...
    def AUG_SCORE(self):
        ...
        meta_tokens_list, _ = self.tokens_list                                          # [14, valN, 512], [13, valN, 512]
        original_meta_token = meta_tokens_list[0,:].cpu().detach().numpy()              # [valN, 512] original meta token            
        val_N = meta_tokens_list.size(1)
        origin_label = self.label.cpu()
        ...
        ## 군집별 점수, 전체 실루엣 점수 
        silhouette_avg_all = silhouette_score(original_meta_token, origin_label)
        # print(f'silhouette_avg_all: ', silhouette_avg_all) 

        ## 각 데이터별 실루엣 점수 -> class별 실루엣 점수 
        silhouette_values = silhouette_samples(original_meta_token, origin_label)
        cluster_silhouette_avg_list = []
        for label in range(num_labels):
            indices = np.where(origin_label == label)[0]        # label이 label인 것들의 인덱스들 
            cluster_silhouette_values = silhouette_values[indices]
            cluster_silhouette_avg = np.mean(cluster_silhouette_values)
            cluster_silhouette_avg_list.append(cluster_silhouette_avg)
            # print(f"class {label}의 평균 실루엣 점수: {cluster_silhouette_avg}")

        # pdb.set_trace()

        plt.figure(figsize=(12, 6)) 
        os.makedirs(f'{self.output_dir}/silhouette_scores_class', exist_ok=True)
        plt.bar(unique_labels, cluster_silhouette_avg_list, color='skyblue')
        # plt.xticks(rotation=45, ha='right')  # x축 레이블을 45도 기울여서 표시합니다.
        plt.title(f"epoch_{self.epoch+1} Meta token's Silhouette Scores for Class")
        plt.xlabel(f'Class (average={silhouette_avg_all:.2f})') ; plt.ylabel('Silhouette Score')
        for i, score in enumerate(cluster_silhouette_avg_list):
            plt.text(i, score + 0.01, f'{score:.2f}', ha='center', va='bottom')
        plt.savefig(f'{self.output_dir}/silhouette_scores_class/delta_{self.epoch+1}.png', dpi=100, bbox_inches='tight')
