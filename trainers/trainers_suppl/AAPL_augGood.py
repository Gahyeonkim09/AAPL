...
class DiVE(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # self.augment_types = ['color_jitter_red', 'color_jitter_green', 'color_jitter_blue', 'grayscale', 'gaussian_noise', 'gaussian_blur','horizontal_flip', 'vertical_flip', 'rotation_90', 'rotation_180', 'rotation_270', 'crop', 'cutout']
        self.augment_types = ['grayscale', 'gaussian_noise', 'gaussian_blur','horizontal_flip', 'crop', 'cutout', 'sobel_filter']
        self.image_distortion = DistortImage()
        self.augtype_meta_tokens = []                       # for the augmetation analysis
        
        # random initialization (learnable attribute)
    ...
    def forward(self, image, label=None):    
        # setting A1: Delta meta tokens , CE Loss(1) + AdTriplet Loss
        ...
        if self.training:
            ...
            # AdTriplet Loss (Delta)
            ad_tri_loss_1, pos_dist_ad_1, neg_dist_ad_1 = self.triplet_loss(delta_meta_1A, delta_meta_2A, delta_meta_1B)   # anchor, pos, neg
            ad_tri_loss_2, pos_dist_ad_2, neg_dist_ad_2 = self.triplet_loss(delta_meta_2B, delta_meta_1B, delta_meta_2A)
            ad_tri_loss = ad_tri_loss_1 + ad_tri_loss_2
            
            # CE Loss(1)
            ce_loss_1 = F.cross_entropy(logit_1, label)
            ce_loss = ce_loss_1
            
            distances = [pos_dist_ad_1, neg_dist_ad_1, pos_dist_ad_2, neg_dist_ad_2]
            total_loss = ad_tri_loss + ce_loss                                               # Total loss = Triplet Loss + Cosine Similarity Loss

            return total_loss, ce_loss, ad_tri_loss, distances
...
class APL_SET_A1_good(TrainerX):
    def __init__(self, cfg):
        super().__init__(cfg)
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            with autocast():
                total_loss, ce_loss, ad_tri_loss, distances = model(image, label)
            ...
            # pos_dist_ad_1, neg_dist_ad_1, pos_dist_ad_2, neg_dist_ad_2
            self.pos_dist_ad_1 = distances[0]
            self.neg_dist_ad_1 = distances[1]
            self.pos_dist_ad_2 = distances[2]
            self.neg_dist_ad_2 = distances[3]
    ...
    def AUG_SCORE(self):
        ...
        # augment_types = ['color_jitter_red', 'color_jitter_green', 'color_jitter_blue', 'grayscale', 'gaussian_noise', 
        # 'gaussian_blur','horizontal_flip', 'vertical_flip', 'rotation_90', 'rotation_180', 'rotation_270', 'crop', 'cutout']
        ##### 일부만 tsne로 뽑기 
        # tsne_types = ['color_jitter_red', 'color_jitter_green', 'color_jitter_blue', 'vertical_flip', 'rotation_90', 'rotation_180', 'rotation_270']
        # indexes = [0, 1, 2, 7, 8, 9, 10]
        # delta_list = delta_meta_list.cpu().detach()
        # delta_combined_list = np.concatenate([delta_list[i, :] for i in indexes])
             
        # tsne_model = TSNE(n_components=2, perplexity=10, verbose=1, random_state=123)
        # tsne_results = tsne_model.fit_transform(delta_combined_list)
        # colors = cm.rainbow(np.linspace(0, 1, len(tsne_types) + 1))

        # plt.figure(figsize=(12, 8))
        # for i, aug_type in enumerate(tsne_types):
        #     indices = (i*val_N, (i+1)*val_N)
        #     plt.scatter(tsne_results[indices[0]:indices[1], 0], tsne_results[indices[0]:indices[1], 1], color=colors[i], label=aug_type)
        # plt.title('Delta_meta_tokens')
        # plt.legend()


        # os.makedirs(f'{self.output_dir}/tsne_delta_part', exist_ok=True)
        # plt.savefig(f'{self.output_dir}/tsne_delta_part/{self.epoch+1}_delta_part.png')
